...
import { Clock, Calendar, FileText, MapPin, User, Mail, Phone } from 'lucide-react';
...
const StudentProfileCard = ({ profile }) => {
  ...
  const currentRegistration = profile.dangKys && profile.dangKys.length > 0 ? profile.dangKys[0] : null;
  const hasActiveRegistration = currentRegistration && ['CHO_DUYET', 'DUYET', 'DANG_O'].includes(currentRegistration.TrangThai);
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              ...
              <Badge className={getStatusInfo(currentRegistration.TrangThai).color}>
          ...
          <CardContent className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-sm text-blue-800 mb-2">
                <strong>Trạng thái:</strong> {getStatusInfo(currentRegistration.TrangThai).description}
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-gray-900 font-medium">{formatDate(currentRegistration.NgayDangKy)}</p>
              </div>
              ...
              {currentRegistration.NgayNhanPhong && (
                <div>
                  ...
                  <p className="text-gray-900 font-medium">{formatDate(currentRegistration.NgayNhanPhong)}</p>
                </div>
              )}
              ...
              {currentRegistration.NgayKetThucHopDong && (
                <div>
                  ...
                  <p className="text-gray-900 font-medium">{formatDate(currentRegistration.NgayKetThucHopDong)}</p>
                </div>
              )}
              ...
              {currentRegistration.Phong && (
                <div>
                  ...
                  <p className="text-gray-900 font-medium">{currentRegistration.Phong.SoPhong}</p>
                </div>
              )}
            </div>
            ...
            {currentRegistration.NguyenVong && (
              <div>
                ...
                <div className="bg-gray-50 border rounded-lg p-3 mt-1">
                  <p className="text-gray-700 text-sm">{currentRegistration.NguyenVong}</p>
                </div>
              </div>
            )}
            ...
            {currentRegistration.TrangThai === 'CHO_DUYET' && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <p className="text-sm text-yellow-800">
                  <strong>Lưu ý:</strong> Đơn đăng ký của bạn đang được xem xét. Chúng tôi sẽ thông báo kết quả qua email trong thời gian sớm nhất.
                </p>
              </div>
            )}
          </CardContent>
      ...
      {!hasActiveRegistration && (
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">Chưa có đăng ký ký túc xá</h3>
