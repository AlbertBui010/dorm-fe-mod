...
const DonGiaDienNuocManagementPage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "NgayKetThuc",
      render: (value) =>
        value ? new Date(value).toLocaleDateString("vi-VN") : "Đang áp dụng",
    },
    {
      header: "Người tạo",
      key: "NguoiTao",
  ...
  const loadStats = () => {
    if (donGias && donGias.length > 0) {
      ...
      const current = donGias.filter(
        (d) => !d.NgayKetThuc || d.NgayKetThuc === null
      ).length;
      const expired = donGias.filter(
        (d) => d.NgayKetThuc && d.NgayKetThuc !== null
      ).length;
      ...
      const avgElectricity =
        total > 0 ? Math.round(totalElectricityPrice / total) : 0;
  ...
  const handleEdit = async (donGia) => {
    try {
      ...
      const canEditResponse = await donGiaDienNuocService.checkCanEdit(
        donGia.MaDonGia
      );
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !formData.NgayApDung ||
      !formData.GiaDienPerKWh ||
      !formData.GiaNuocPerM3
    ) {
    ...
    if (
      parseFloat(formData.GiaDienPerKWh) < 0 ||
      parseFloat(formData.GiaNuocPerM3) < 0
    ) {
    ...
    try {
      ...
      let response;
      if (editingDonGia) {
        response = await donGiaDienNuocService.updateDonGia(
          editingDonGia.MaDonGia,
          submitData
        );
      } else {
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              Quản lý đơn giá Điện/Nước
            </h1>
            <p className="text-gray-600">
              Quản lý giá điện và nước áp dụng trong ký túc xá
            </p>
          </div>
          ...
          <Card className="p-4">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {stats.avgWater
                    ? new Intl.NumberFormat("vi-VN").format(stats.avgWater)
                    : 0}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <p className="font-medium">
                  {new Date(currentDonGia.NgayApDung).toLocaleDateString(
                    "vi-VN"
                  )}
                </p>
              </div>
