...
const RegistrationApprovalPage = () => {
  ...
  const viewRegistrationDetail = async (registration) => {
    try {
      ...
      const roomsResponse =
        await registrationApprovalService.findAvailableRooms(
  ...
  const columns = [
    {
      ...
      render: (value, row) => (
        <div className="flex items-center">
          ...
          <div className="ml-4">
            <div className="text-sm font-medium text-gray-900">{row.HoTen}</div>
            <div className="text-sm text-gray-500">{row.MaSinhVien}</div>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (value, row) =>
        new Date(row.NgayNhanPhong).toLocaleDateString("vi-VN"),
    },
    {
      key: "NguyenVong",
      header: "Nguyện vọng",
      render: (value, row) => (
        <Badge variant="secondary">{row.NguyenVong}</Badge>
      ),
    },
    {
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              {filters.trangThai
                ? "Xem và quản lý các đăng ký ký túc xá theo trạng thái"
                : "Duyệt và quản lý các đăng ký ký túc xá của sinh viên"}
            </p>
          </div>
        </div>{" "}
        {/* Statistics Cards */}
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-md p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900">
                    {new Date(
                      selectedRegistration.NgayNhanPhong
                  ...
                  {availableRooms.length === 0 ? (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-md p-6">
                      <p className="text-yellow-800 text-center">
                        Không có phòng trống phù hợp với giới tính{" "}
                        {selectedRegistration.GioiTinh}
                      </p>
                    </div>
                  ) : (
                        ...
                        <Card key={room.MaPhong} className="p-6">
                          <div className="mb-4">
                            ...
                            <p className="text-sm text-gray-600 mb-1">
                              {room.LoaiPhong} • {room.SucChua} giường •{" "}
                              {room.availableBeds} trống
                            </p>
                            <p className="text-sm text-indigo-600 font-medium">
                              {parseFloat(room.GiaThueThang).toLocaleString(
                                "vi-VN"
                              )}{" "}
                              VNĐ/tháng
                            </p>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {room.emptyBedDetails?.map((bed) => (
                                <Button
                                  key={bed.MaGiuong}
                                  variant={
                                    approvalForm.maGiuong === bed.MaGiuong
                                      ? "default"
                                      : "outline"
                                  }
                                  size="sm"
                ...
                {availableRooms.length > 0 && (
                  <div className="bg-green-50 border border-green-200 rounded-md p-6">
                    ...
                    <Button
                      ...
                      disabled={
                        !approvalForm.maPhong ||
                        !approvalForm.maGiuong ||
                        loading
                      }
