...
const StudentPaymentPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [profileResult, paymentsResult, statsResult] = await Promise.all([
  ...
  const handlePaymentAction = async (payment, method) => {
    ...
    try {
      if (method === 'online') {
        const result = await studentPaymentService.createPaymentLink(payment.MaThanhToan);
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      ...
      'CHO_XAC_NHAN_TIEN_MAT': { text: 'Chờ xác nhận tiền mặt', variant: 'purple' },
    ...
    <div className="min-h-screen bg-gray-50">
              ...
              <div className="flex flex-wrap gap-4 items-center">
                <div>
                  ...
                  >
                    ...
                    <option value="DANG_CHO_THANH_TOAN">Đang chờ thanh toán</option>
                    <option value="CHO_XAC_NHAN_TIEN_MAT">Chờ xác nhận tiền mặt</option>
            ...
            {payments.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">
                  {statusFilter ? 'Không có thanh toán nào với trạng thái này.' : 'Bạn chưa có khoản thanh toán nào.'}
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {payments.map((payment) => (
                  <div key={payment.MaThanhToan} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm text-gray-600">
                          <div>
                            <span className="font-medium">Phòng:</span> {payment.Phong?.SoPhong || 'N/A'}
                          </div>
                          <div>
                            <span className="font-medium">Tháng:</span> {payment.ThangNam}
                          </div>
                          <div>
                          ...
                          <div>
                            <span className="font-medium">Hạn thanh toán:</span> {formatDate(payment.NgayTao)}
                          </div>
                        </div>
                      </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-900">{selectedPayment.MaThanhToan}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Loại thanh toán</label>
                  <p className="text-gray-900">{selectedPayment.LoaiThanhToan}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Phòng</label>
                  <p className="text-gray-900">{selectedPayment.Phong?.SoPhong || 'N/A'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Tháng/năm</label>
                ...
                <div>
                  ...
                  <p className="text-gray-900">{selectedPayment.HinhThuc || 'Chưa xác định'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Ngày thanh toán</label>
                  <p className="text-gray-900">{formatDate(selectedPayment.NgayThanhToan)}</p>
                </div>
              </div>
            </div>
          )}
      ...
      <Modal isOpen={showPaymentModal} onClose={() => setShowPaymentModal(false)}>
              ...
              <div className="space-y-3">
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div>
                      <h4 className="font-medium text-gray-900">Chuyển khoản online</h4>
                      <p className="text-sm text-gray-500">
        ...
        <ModalFooter>
          ...
          <Button
            onClick={() => handlePaymentAction(selectedPayment, paymentMethod)}
            variant="primary"
